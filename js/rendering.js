...
function drawTunnelBackground(){
  ...
  for(let i=0;i<70;i++){
    const x=((i*173-worldTime*.015)%W+W)%W;
    const y=90+((i*71)%Math.max(1,H-150));
    ctx.fillStyle="rgba(190,220,210,.08)";
...
function drawRain(){
  ...
  ctx.save();
  ctx.strokeStyle="rgba(190,225,220,.17)";
  ctx.lineWidth=1;
  for(const r of rain){
    ctx.beginPath();
    ctx.moveTo(r.x,r.y);
    ctx.lineTo(r.x+r.drift,r.y+r.length);
    ctx.stroke();
    r.y+=r.speed;
    r.x+=r.drift;
...
function drawPlayerShadow(){
  ...
  for(const p of list){
    if(centerX>=p.x&&centerX<=p.x+p.w&&p.y>=player.y+player.h-2){
  ...
  ctx.save();
  ctx.globalAlpha=.25*scale;
  ctx.fillStyle="#000";
  ctx.beginPath();
  ctx.ellipse(centerX,groundY+3,22*scale,5*scale,0,0,Math.PI*2);
  ctx.fill();
  ctx.restore();
}

function resetSurvivorFollowers(){
  survivorFollowers.forEach((follower,index)=>{
    const row=Math.floor(index/2);
    const side=index%2?1:-1;
    follower.x=player.x-player.facing*(64+row*30)+side*10;
    follower.y=player.y+player.h-42+row*3;
...
function updateSurvivorFollowers(){
  ...
  survivorFollowers.forEach((follower,index)=>{
    const row=Math.floor(index/2);
    const side=index%2?1:-1;
    const distance=64+row*30;
    const targetX=player.x-player.facing*distance+side*10;
    const targetY=player.y+player.h-42+row*3;
    follower.vx=(follower.vx+(targetX-follower.x)*.045)*.78;
    follower.vy=(follower.vy+(targetY-follower.y)*.06)*.72;
...
function drawRescuedSurvivors(){
  if(!survivorsFollowing||tunnelMode)return;
  survivorFollowers.forEach(follower=>{
    const bob=Math.sin(player.anim+follower.phase)*1.8;
    const stride=Math.sin(player.anim+follower.phase)*2;
    ctx.save();
    ctx.translate(follower.x,follower.y+bob);
    ctx.fillStyle="#111b1d";
    ctx.fillRect(0,0,22,42);
    ctx.fillStyle="#0a1012";
    ctx.fillRect(3,38,5,7+Math.max(0,stride));
    ctx.fillRect(14,38,5,7+Math.max(0,-stride));
    ctx.fillStyle="#111b1d";
    ctx.beginPath();
    ctx.arc(11,-7,10,0,Math.PI*2);
    ctx.fill();
    ctx.fillStyle="#b9efc8";
    ctx.fillRect(player.facing>0?13:3,-9,5,2);
    ctx.restore();
  });
}
...
function draw(){
  ...
  if(tunnelMode)drawTunnelWorld();
  else drawSurfaceWorld();
  ctx.restore();
...
function drawSurfaceWorld(){
  for(let i=-5;i<50;i++){
    ...
    ctx.fillRect(x,-300,230,1100);
  }
  platforms.forEach((p,i)=>{
    ctx.fillStyle=i>=10?"#263b40":"#293d42";
    ctx.fillRect(p.x,p.y,p.w,camY+H-p.y);
  ...
  scraps.forEach(s=>{
    ...
    ctx.save(); ctx.translate(s.x,s.y); ctx.rotate(Math.sin(worldTime/300+s.x)*.2);
    ctx.fillStyle="#e5c76f"; ctx.shadowBlur=12; ctx.shadowColor="#e5c76f";
    ctx.fillRect(-6,-5,12,10); ctx.fillStyle="#fff1a8"; ctx.fillRect(-3,-5,3,10); ctx.restore();
  });
  items.forEach(it=>{
    if(it.collected)return;
    ctx.fillStyle="#d9f8df"; ctx.shadowBlur=20; ctx.shadowColor="#b9efc8";
    ctx.beginPath(); ctx.arc(it.x,it.y,9+Math.sin(Date.now()/150)*2,0,Math.PI*2); ctx.fill(); ctx.shadowBlur=0;
  });
  ctx.fillStyle="#51686a";
  ctx.fillRect(3510,150,16,350); ctx.fillRect(3590,150,16,350);
  ctx.fillStyle=components>=3?"#d9f8df":"#657879";
  ctx.shadowBlur=components>=3?30:5; ctx.shadowColor="#b9efc8";
  ctx.beginPath(); ctx.arc(3558,125,23,0,Math.PI*2); ctx.fill(); ctx.shadowBlur=0;
  if(stage>=6){
    for(let i=0;i<55;i++){
      const x=4000+i*70,y=445-(i%4)*28;
      ctx.fillStyle="rgba(185,239,200,.7)"; ctx.shadowBlur=10; ctx.shadowColor="#b9efc8";
      ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fill();
    }
    ctx.shadowBlur=0;
  }
  if(stage>=7){
    ctx.fillStyle="#111b1d"; ctx.fillRect(4535,418,30,58); ctx.beginPath(); ctx.arc(4550,405,16,0,Math.PI*2); ctx.fill();
    ctx.fillStyle="#b9efc8"; ctx.fillRect(4558,402,7,3);
  }
  if(stage>=8){
    ...
    ctx.fillStyle="rgba(185,239,200,.4)"; ctx.fillRect(6130,335,130,4);
    for(let i=0;i<7;i++){ctx.fillStyle="#314346";ctx.fillRect(6150+i*10,500-i*9,100-i*20,8);}
  }
  ...
  if(player.grapple){
    ctx.strokeStyle="#d6eee6"; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(player.x+14,player.y+20); ctx.lineTo(player.grapple.x,player.grapple.y); ctx.stroke();
  }
...
function drawTunnelWorld(){
  ...
  for(let x=-300;x<9000;x+=280){
    ...
    ctx.fillStyle="#111e21"; ctx.beginPath();
    ...
    ctx.strokeStyle="rgba(105,145,135,.18)"; ctx.lineWidth=10; ctx.beginPath(); ctx.moveTo(x+60,120); ctx.lineTo(x+60,500); ctx.stroke();
    ctx.strokeStyle="rgba(190,230,210,.12)"; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(x+100,160); ctx.lineTo(x+220,160); ctx.stroke();
  }
  ...
  for(let x=900;x<8000;x+=700){
    ctx.fillStyle="rgba(180,90,70,.18)"; ctx.fillRect(x,250,80,30);
    ctx.fillStyle="rgba(220,130,100,.5)"; ctx.fillRect(x+15,262,50,3);
  }
  if(stage>=11){
    ctx.fillStyle="rgba(120,170,150,.08)"; ctx.fillRect(4850,360,900,210);
    for(let i=0;i<6;i++){
      const x=5000+i*110; ctx.fillStyle="#263b3c"; ctx.beginPath(); ctx.moveTo(x,500); ctx.lineTo(x+50,410); ctx.lineTo(x+100,500); ctx.closePath(); ctx.fill();
      ctx.strokeStyle="rgba(180,220,205,.15)"; ctx.stroke();
    }
  ...
  if(stage>=12){
    ctx.fillStyle="#071012"; ctx.fillRect(7850,180,260,250); ctx.strokeStyle="#829890"; ctx.lineWidth=5; ctx.strokeRect(7850,180,260,250);
  ...
  tunnelAnchors.forEach(a=>{
    const pulse=8+Math.sin(Date.now()/180)*2;
    ctx.strokeStyle=player.grapple===a?"rgba(220,255,240,.9)":"rgba(150,240,220,.35)";
    ctx.lineWidth=player.grapple===a?3:1; ctx.beginPath(); ctx.arc(a.x,a.y,pulse,0,Math.PI*2); ctx.stroke();
...
function drawPlayer(){
  drawPlayerShadow();
  const moving=Math.min(1,Math.abs(player.vx)/player.speed);
  const bob=Math.sin(player.anim*.42)*(1.1+moving*1.1);
  const tilt=Math.max(-.08,Math.min(.08,player.vx*.012));
  ctx.save(); ctx.translate(player.x+14,player.y+24+bob); ctx.rotate(tilt);
  ...
  ctx.fillStyle="#31464a"; ctx.fillRect(player.facing>0?-14:9,-6,5,17);
  ctx.restore();
}
...
function update(){
  ...
  if(tunnelMode){
    camX=Math.max(-100,Math.min(13200,camX));
  ...
  updatePrompt();
  particles.forEach(q=>{q.x+=q.vx;q.y+=q.vy;q.vy+=.08;q.life--;});
